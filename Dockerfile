...
# Pivnet CLI
RUN curl -L -k -o /usr/local/bin/pivnet https://github.com/pivotal-cf/pivnet-cli/releases/download/v3.0.1/pivnet-linux-amd64-3.0.1  && \
    chmod 755 /usr/local/bin/pivnet

# Tanzu CLI
COPY tanzu-framework-linux-amd64.tar /tmp
COPY mck-ca.pem /usr/local/share/ca-certificates/mck-ca.crt
...
RUN echo 'Acquire::https::repos.influxdata.com::Verify-Peer "false";' >> /etc/apt/apt.conf.d/99influxdata-cert
...
( \
  ...
  KREW="krew-${OS}_${ARCH}" && \
  curl -kfsSLO "https://github.com/kubernetes-sigs/krew/releases/latest/download/${KREW}.tar.gz" && \
